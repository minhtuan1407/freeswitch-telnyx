 ...
 *
 *
 * mod_telnyx_demo.c -- Framework Demo Module
 *
 */
#include <switch.h>

/* Prototypes */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_telnyx_demo_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_telnyx_demo_runtime);
SWITCH_MODULE_LOAD_FUNCTION(mod_telnyx_demo_load);

/* SWITCH_MODULE_DEFINITION(name, load, shutdown, runtime)
 * Defines a switch_loadable_module_function_table_t and a static const char[] modname
 */
SWITCH_MODULE_DEFINITION(mod_telnyx_demo, mod_telnyx_demo_load, mod_telnyx_demo_shutdown, NULL);
...
static switch_status_t config_callback_siptrace(switch_xml_config_item_t *data,
...
static switch_xml_config_enum_item_t config_opt_codec_negotiation_enum[] = {{"greedy", CODEC_NEGOTIATION_GREEDY},
...
static switch_xml_config_item_t instructions[] = {
	/* parameter name        type                 reloadable   pointer                         default value     options
	   structure */
	SWITCH_CONFIG_ITEM("codec-negotiation-str", SWITCH_CONFIG_STRING, CONFIG_RELOADABLE, &globals.codec_negotiation_str,
					   "greedy", &config_opt_codec_negotiation, "greedy|generous|evil",
					   "Specifies the codec negotiation scheme to be used."),
	SWITCH_CONFIG_ITEM("codec-negotiation", SWITCH_CONFIG_ENUM, CONFIG_RELOADABLE, &globals.codec_negotiation,
					   (void *)CODEC_NEGOTIATION_GREEDY, &config_opt_codec_negotiation_enum, "greedy|generous|evil",
					   "Specifies the codec negotiation scheme to be used."),
	SWITCH_CONFIG_ITEM_CALLBACK("sip-trace", SWITCH_CONFIG_BOOL, CONFIG_RELOADABLE, &globals.sip_trace,
								(void *)SWITCH_FALSE, (switch_xml_config_callback_t)config_callback_siptrace, NULL,
								"yes|no", "If enabled, print out sip messages on the console."),
	SWITCH_CONFIG_ITEM("integer", SWITCH_CONFIG_INT, CONFIG_RELOADABLE, &globals.integer, (void *)100,
					   &config_opt_integer, NULL, NULL),
...
static switch_status_t do_config(switch_bool_t reload)
{
	...
	if (switch_xml_config_parse_module_settings("telnyx_demo.conf", reload, instructions) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "Could not open telnyx_demo.conf\n");
		return SWITCH_STATUS_FALSE;
	} else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "telnyx_demo.conf configuration open complete\n");
		return SWITCH_STATUS_SUCCESS;
	}
...
#define _switch_stun_packet_next_attribute(attribute, end)                                                             \
	(attribute && (attribute = (switch_stun_packet_attribute_t *)(attribute->value + ntohs(attribute->length))) &&     \
	 ((void *)attribute < end) && ntohs(attribute->length) && ((void *)(attribute + ntohs(attribute->length)) < end))

#define _switch_stun_attribute_padded_length(attribute)                                                                \
	((uint16_t)(ntohs(attribute->length) + (sizeof(uint32_t) - 1)) & ~sizeof(uint32_t))
...
SWITCH_STANDARD_API(telnyx_demo_function)
{
...
static void mycb(switch_core_session_t *session, switch_channel_callstate_t callstate, switch_device_record_t *drec)
{
	...
	switch_log_printf(
		SWITCH_CHANNEL_CHANNEL_LOG(channel), SWITCH_LOG_CRIT,
...
/* Macro expands to: switch_status_t mod_telnyx_demo_load(switch_loadable_module_interface_t **module_interface,
...
SWITCH_MODULE_LOAD_FUNCTION(mod_telnyx_demo_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	SWITCH_ADD_API(api_interface, "telnyx_demo", "Telnyx Demo API", telnyx_demo_function, "syntax");
...
/*
  ...
  Macro expands to: switch_status_t mod_telnyx_demo_shutdown() */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_telnyx_demo_shutdown)
{
...
/*
  ...
  Macro expands to: switch_status_t mod_telnyx_demo_runtime()
SWITCH_MODULE_RUNTIME_FUNCTION(mod_telnyx_demo _runtime)
{
	while(looping)
	{
		switch_cond_next();
